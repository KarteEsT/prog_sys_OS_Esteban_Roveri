public class Inode {
    ...
    public void writeToMemory(int fileType, int fileSize, long creationTime, 
		...
		// Pour vérifier qu'on est bien dans le tableau d'inode
		if (offset > INODE_TABLE_OFFSET || offset < INODE_TABLE_OFFSET + INODE_SIZE) {
			...
			for (int i = 0; i < DIRECT_POINTERS; i++) {
				offset += Utils.writeLong(memory, offset, directPointers[i]);         
			}
			// Écrire le pointeur indirect
			offset += Utils.writeLong(memory, offset, indirectPointer);
			// Écrire les permissions
			offset += Utils.writeLong(memory, offset, permissions);
			// Écrire le nombre de liens
            offset += Utils.writeLong(memory, offset, linkCount);
		}
